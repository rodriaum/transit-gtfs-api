    ...
    public async Task LoadDataFromFilesAsync()
    {
        try
        {
            ...
            for (int i = 0; i < gtfsDirectories.Count; i++)
            {
                ...
                GtfsData? gtfsData = Constant.GtfsDataList.Find(it => it.AgencyKey == agencyKeyByPath);
                ...
                string? agencyKey = gtfsData.AgencyKey;
                ...
                {
                    _logger.LogInformation($"Ignoring {gtfsData.AgencyKey} becauses already exists.");
                    continue;
                }
